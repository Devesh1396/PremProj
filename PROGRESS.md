...
**K06: the no-captions payload is captured too, and it is the nastier
shape.** Sampled 2026-09-11 —
`testing/fixtures/apify/youtube_no_captions.json`, loaded by the suite
rather than retyped.
...
there is no transcript at all, and a flag read without first confirming
segments exist records a **failed fetch as the highest-quality transcript
type there is** — which would then also keep those claims off
`v_asr_derived_claims`. `auto_generated_flag()` is the one place that field
is read anywhere in the codebase, and it returns `None` without segments.
...
reaches the inbox, not even a sidecar**. The suite proves the negative
downstream too: no envelope exists for that video, so nothing recorded it
as a human transcript, and a second run neither duplicates the row nor
moves its status.

**Both paths are now written against observed behaviour**, not an assumed
shape on either. The two mechanisms were mutation-tested before being
trusted: reading `isAutoGenerated` unconditionally, and ignoring `error`,
...
- **A bad or removed video ID has never been observed.** It may produce a
  different shape again — plausibly an **empty dataset** rather than an
  item carrying `error`. Nothing asserts what it looks like, and it is the
  remaining hole in K06's payload coverage.
  *(The no-captions case is no longer on this list: captured 2026-09-11,
  see below.)*
