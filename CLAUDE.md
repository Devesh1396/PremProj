...
**The no-captions payload has NEVER been observed.** Nothing asserts its
shape. What is enforced is shape-independent: no usable segments means no
transcript, `FULL_TEXT_NOT_AVAILABLE`, nothing delivered. **An empty string
must never travel as a transcript** — it would pass through extraction as a
source that taught us nothing and §54 would agree with it.
