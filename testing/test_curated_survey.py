...
def main() -> int:
    ...
    check("`Decision intelligence` is flagged across the two fixtures",
          "'Decision intelligence'" in out.split("MORE THAN ONE KIND OF CONTAINER")[-1])
    one = run([FIX / "curated" / "t2d_video14.md"])
    check("...and NOT from Video 14 alone, where it occurs in one kind",
          "(none across the files given)" in one)
    syn = run([tmp])
    tail = syn.split("MORE THAN ONE KIND OF CONTAINER")[-1]
